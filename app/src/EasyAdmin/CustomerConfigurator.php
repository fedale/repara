...
final class CustomerConfigurator implements FieldConfiguratorInterface
{
    ...
    public function configure(FieldDto $field, EntityDto $entityDto, AdminContext $context): void
    {
        ...
        $targetCrudControllerFqcn = $field->getCustomOption(CustomerField::OPTION_CRUD_CONTROLLER)
            ?? $context->getCrudControllers()->findCrudFqcnByEntityFqcn($targetEntityFqcn);
        $field->setCustomOption(CustomerField::OPTION_CRUD_CONTROLLER, $targetCrudControllerFqcn);

        if (CustomerField::WIDGET_AUTOCOMPLETE === $field->getCustomOption(CustomerField::OPTION_WIDGET)) {
        ...
        if (\count($propertyNameParts) > 1) {
            ...
            try {
                ...
                $field->setCustomOption(CustomerField::OPTION_RELATED_URL, $this->generateLinkToAssociatedEntity($targetCrudControllerFqcn, $relatedEntityDto));
        ...
        if (true === $field->getCustomOption(CustomerField::OPTION_AUTOCOMPLETE)) {
            $targetCrudControllerFqcn = $field->getCustomOption(CustomerField::OPTION_CRUD_CONTROLLER);
            ...
            $autocompleteEndpointUrl = $this->adminUrlGenerator
                ...
                ->setController($field->getCustomOption(CustomerField::OPTION_CRUD_CONTROLLER))
                ->setAction('autocomplete')
                ->set(CustomerField::PARAM_AUTOCOMPLETE_CONTEXT, [
            ...
            $field->setFormTypeOptionIfNotSet('query_builder', static function (EntityRepository $repository) use ($field) {
                ...
                if ($queryBuilderCallable = $field->getCustomOption(CustomerField::OPTION_QUERY_BUILDER_CALLABLE)) {
    ...
    {
        $field->setCustomOption(CustomerField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toOne');
        ...
        $field->setCustomOption(CustomerField::OPTION_RELATED_URL, $this->generateLinkToAssociatedEntity($targetCrudControllerFqcn, $targetEntityDto));
    ...
    {
        $field->setCustomOption(CustomerField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toMany');
